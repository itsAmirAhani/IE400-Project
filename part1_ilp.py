...
part1_model = gp.Model("part1_model")

x={}
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        x[i, j] = part1_model.addVar(vtype=GRB.BINARY, name=f"x_{i}_{j}") # Binary variable. 1 if seeker i is assigned to job j, 0 otherwise.

# Constraint for assigning at most 1 job to every seeker.
for i in seekers['Seeker_ID']:
    part1_model.addConstr(sum(x[i, j] for j in jobs['Job_ID']) <= 1) 
...
for j in jobs['Job_ID']:
    ...
    part1_model.addConstr(sum(x[i, j] for i in seekers['Seeker_ID']) <= num_positions) 
...
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        ...
        part1_model.addConstr(x[i, j] <= job_type_constr)
...
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        ...
        part1_model.addConstr(x[i, j] <= salary_constr)
...
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        ...
        part1_model.addConstr(x[i, j] <= requirement_constr)
...
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        ...
        part1_model.addConstr(x[i, j] <= experience_constr)
...
for i in seekers['Seeker_ID']:
    for j in jobs['Job_ID']:
        ...
        part1_model.addConstr(x[i, j] <= location_constr)

part1_model.setObjective(sum(x[i, j] * int(jobs[jobs['Job_ID'] == j]['Priority_Weight'].iloc[0]) for i in seekers['Seeker_ID'] for j in jobs['Job_ID']), GRB.MAXIMIZE)

part1_model.optimize()

# Checking results 
total_positions = jobs['Num_Positions'].sum()
print("Total available positions:", total_positions)
